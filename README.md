...
- 搜索结果直接展示分享率范围、最旧添加时间、累计上传/下载量、最近同步时的速度、下载器和存放路径；展开可核对每个任务的数据与最后同步时间。
- 路径分类取规范化内容路径的父目录名，例如 `/volume1/001/media/Movies/The.Whisper.Man.2026` 归为 `Movies`；同组有多个分类时全部保留，根目录下的内容显示为未分类。
- 按规范化路径、大小和已选文件清单建立自动分组。
- 支持逻辑分组的手动合并、拆分、成员移动、锁定、恢复自动分组和并发安全撤销。
- 高级搜索条件可命名保存为服务端预设，跨浏览器使用；搜索栏下拉勾选后统一应用，多选表示同时满足，保留每个预设内部的条件关系。支持删除预设，清空筛选不会删除已保存的预设。
- 多选支持当前页全选、跨页保留和清空选择；切换筛选条件时清空勾选。批量删除会合并选中组的任务，一次预览文件影响、确认并跟踪执行进度。
- 删除前自动检查任务与文件影响，在同一个窗口确认后执行，服务端保留引用检查和状态复验。
- 管理下载器、物理存储身份、跨主机路径映射和 Tracker 归类规则。
- 每日同步 IYUU 站点元数据目录；失败时保留上次成功快照，且不会把网站域名误当作 Tracker announce 域名。
- 右下角圆环浮框跟踪删除步骤与最终校验，支持收起、跨页查看及当前标签页刷新恢复。
- 审计记录支持按操作和结果筛选、服务端分页和完整详情，并可重新打开删除执行记录。
